...
class Orchestrator:
    ...
    async def _review_criteria(self, criteria_path: str) -> str:
        ...
        prompt = (
            ...
            f"STEP 3 — DEFENSIVE UI CHECKLIST (first-build priority):\n"
            f"Include these four UI states in your initial component code — they are "
            f"commonly missed on first builds:\n"
            f"  1. Skeleton loading state for every data-fetching component (not just a spinner)\n"
            f"  2. Error banner with retry button for API failures (no silent failures)\n"
            f"  3. Loading/disabled state for form submit buttons during requests\n"
            f"  4. Meaningful empty state for every list/table (with CTA, not just 'No data')\n"
            f"Do NOT try to verify these at runtime (no dev server / curl). "
            f"Write the code once; the Evaluator will test them.\n\n"
            f"STEP 4 — Confirm:\n"
