...
4. **Design language is essential — be BOLD and DIFFERENT.** Apply the frontend design skill you read, especially the Anti-Convergence Rules and Direction Catalog. Pick from the catalog and check your choices against the convergence traps. If you catch yourself writing "dark mode + gold accent + serif display font + noise texture", STOP — that's the #1 AI design cliche. The app must look like it was designed by a human with strong opinions — not generated by AI. Generic = failure.
